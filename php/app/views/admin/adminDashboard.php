    ...
    <style>
        ...
        @media (max-width: 768px) {
            .main-content {
                ...
                /* Remove sidebar margin on mobile when sidebar is closed */
                transition: margin-left 0.3s ease-in-out;
            }

            .main-content.sidebar-open {
                margin-left: 16rem;
                /* Match sidebar width (w-64 = 16rem) when sidebar is open */
            }
            ...
            .users-table thead {
                display: none;
                /* Hide table headers on mobile for a card-like layout */
            }
            ...
            .pagination-section .pagination-buttons {
                ...
                gap: 0.5rem;
                /* Added gap between pagination buttons */
            }
                ...
                <div class="animate-slide-in">
                    ...
                    <div class="px-8 py-4 border-t border-gray-100">
                        <div class="pagination-section flex items-center justify-between">
                            <!-- Info "Showing X to Y of Z results" tetap sama (jika diperlukan) -->
                            <div class="text-sm text-gray-500">
                                Showing
                                <span class="font-medium"><?= ($data['currentPage'] - 1) * $data['limit'] + 1; ?></span>
                                to
                                <span class="font-medium"><?= min($data['currentPage'] * $data['limit'], $data['totalUsers']) ?></span>
                                of
                                <span class="font-medium"><?= $data['totalUsers'] ?></span>
                                results
                            </div>

                            <!-- Pagination Buttons dengan ellipsis -->
                            <div class="pagination-buttons flex items-center space-x-2" id="pagination">
                                ...
                                // Fungsi untuk mencetak link halaman
                                function renderPageLink($page, $currentPage)
                                {
                                    $activeClass = ($page == $currentPage)
                                        ? 'border border-red-400 text-red-500'
                                        : 'bg-white text-red-500 duration-300 hover:bg-red-100';
                                    echo '<a href="?page=' . $page . '" class="px-4 py-2 text-sm font-medium rounded-lg ' . $activeClass . '">' . $page . '</a>';
                                }

                                if ($totalPages <= 5) {
                                    // Tampilkan semua halaman jika total halaman ≤ 5
                                    for ($i = 1; $i <= $totalPages; $i++) {
                                        renderPageLink($i, $currentPage);
                                    }
                                } else {
                                    // Tampilkan halaman pertama
                                    renderPageLink(1, $currentPage);

                                    // Jika halaman aktif lebih besar dari 3, tampilkan ellipsis
                                    if ($currentPage > 3) {
                                        echo '<span class="px-4 py-2 text-sm font-medium">...</span>';
                                    }

                                    // Tampilkan dua halaman di sekitar halaman aktif (currentPage - 1, currentPage, currentPage + 1)
                                    ...
                                    // Jika halaman aktif kurang dari totalPages - 2, tampilkan ellipsis
                                    if ($currentPage < $totalPages - 2) {
                                        echo '<span class="px-4 py-2 text-sm font-medium">...</span>';
                                    }

                                    // Tampilkan halaman terakhir
                                    renderPageLink($totalPages, $currentPage);
                                }
                                ?>
                            </div>
                        </div>
